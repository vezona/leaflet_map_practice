...
export default {
  setup() {
    ...
    // 設定 point 的樣式
    function returnMarker (attributes, layer){
        console.log(attributes.properties.name);
        if (attributes.properties.name !== undefined){
           return L.circleMarker(layer, { radius: 20, color: 'deeppink' })
                   .bindTooltip(`<p>${attributes.properties.name}</p>`)
        } else {
